...
- 2026-08-30 — **The systems rework merged, tagged and pushed** on Marcus's call: `systems-fixed-frame` merged `--no-ff` onto a `main` that had not moved, so the merged tree is byte-identical to the branch tip that tested green (full suite 2478 re-run on `main` before the tag). That is D2, E1 and E2 of `docs/UI_REDESIGN.md` plus all four stages of `docs/SYSTEMS_MODE_REWORK.md` — one fixed frame, the item filter, the dissolve, export framing what the stage frames, and the dark ground with an edge. Tagged `v0.2-beta.8`; `main`, the tag and the branch are pushed. Next: Marcus's in-app check of the dark ground and the frame edge in both modes.
- 2026-08-30 — **A ledger line under a rest no longer beats the rest to the page.** Rule 4 makes a rest retrospective — it fires when its silence resolves — but the ink the adapter hangs OFF a rest has its own id, so it was in neither trigger map, missed the group table (built from noteheads only), and fell back to its own onset: the dash lit on the rest's silent beat while the rest waited. `schedule.py` now builds a second group table from the RESTS, and the lookup order is note → rest → notehead group → rest group → own onset. It is a strict fallback after the notehead table, never a merge — a rest and a note cannot share a voice slot at one onset. Voice is normalised through the new public `voice_slot` (None → 0), because a rest can carry voice None while its dash carries layer 0. The fix reaches more than dashes: complex3 has four dotted rests whose augmentation dot was 3.5 beats early. The live oracle's D2 check needed widening to match — it exempted a rest's own positive shift but not its ink's, so it read the fix as four new findings; it now exempts only what lands exactly on a rest's trigger in that rest's own slot, so any other shift is still a finding. 6 new tests (2 synthetic dash, 1 synthetic dot/fermata, 1 complex1 mRest pin, 1 sweep over all five of complex1's rest-owned dashes, 1 complex3 dot pin), all six red on the old code, checked. Goldens untouched (they pin layouts, not schedules). Full suite 2484 green on `fix/rest-attachments`. Awaiting Marcus's in-app check.
- 2026-08-31: Clefs, key signatures and time signatures no longer run an
  effect — they just appear. New policy module
  `core/animation/plain_kinds.py` (PLAIN_KINDS = schedule.SIG_KINDS, plus
  `effect_name_for`); `render/animate.py` runs the resolved name through
