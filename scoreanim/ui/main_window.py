...
class MainWindow(QMainWindow):
    ...
    def _on_document_changed(self) -> None:
        ...
        # SCHEDULE (and its reveal tracks) only — no Verovio, no scene
        # rebuild. After the re-engrave gate: a re-engrave already
        # rebuilt the schedule from the document and reset the loader's
        # cache, so the diff is False on that path.
        if (self.animation_inputs is not None
                and self.loader.needs_reschedule(doc)):
            rebuilt = self.loader.rebuild_schedule(doc)
            if rebuilt is not None:
                schedule, tracks = rebuilt
                self.playback.set_schedule(schedule, tracks)
