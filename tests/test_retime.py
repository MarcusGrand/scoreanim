...
def test_target_is_hit_exactly() -> None:
    events = retime(CURVE, beat=8.0, seconds=4.5, left=4.0, right=12.0)
...
def test_tempo_shape_inside_a_span_is_preserved() -> None:
    ...
    out = retime(events, beat=4.0, seconds=3.0, left=0.0, right=None)
...
def test_ripple_shifts_the_tail_by_one_constant() -> None:
    ...
    events = retime(CURVE, beat=8.0, seconds=4.5, left=4.0, right=None)
...
def test_last_line_ripples_by_definition() -> None:
    ...
    a = retime(CURVE, beat=16.0, seconds=9.0, left=8.0, right=None)
...
def test_a_first_event_after_beat_zero_still_pins_its_anchor() -> None:
    ...
    out = retime(events, beat=8.0, seconds=3.5, left=4.0, right=12.0)
...
def test_anchors_coinciding_with_events(beat, left, right) -> None:
    ...
    out = retime(CURVE, beat=beat, seconds=target, left=left, right=right)
...
def test_a_boundary_a_hair_from_an_event_is_not_duplicated() -> None:
    out = retime(CURVE, beat=8.0 + 1e-12, seconds=4.5, left=4.0, right=12.0)
...
def test_a_no_op_retime_leaves_the_map_alone_and_keeps_user_events() -> None:
    ...
    m = TempoMap(list(events))
    out = retime(events, beat=2.0, seconds=m.seconds_at(2.0), left=0.0,
                 right=4.0)
    assert out == events
...
def test_no_events_raises() -> None:
    with pytest.raises(ValueError):
        retime((), beat=1.0, seconds=1.0, left=0.0, right=2.0)


@pytest.mark.parametrize("right", [12.0, None])
def test_bounds_endpoints_are_reachable_and_saturate_the_bpm_range(right
                                                                  ) -> None:
    lo, hi = retime_bounds(CURVE, beat=8.0, left=4.0, right=right)
    assert lo < hi
    for target in (lo, hi):
        out = retime(CURVE, beat=8.0, seconds=target, left=4.0, right=right)
...
def test_bounds_bracket_the_current_position() -> None:
    m = TempoMap(list(CURVE))
    lo, hi = retime_bounds(CURVE, beat=8.0, left=4.0, right=12.0)
...
def test_beyond_the_bounds_raises() -> None:
    ...
    for target in (lo - 0.01, hi + 0.01):
        with pytest.raises(ValueError):
            retime(CURVE, beat=8.0, seconds=target, left=4.0, right=12.0)
...
def test_property_pins_hold_and_target_is_hit(seed: int) -> None:
    ...
    for _ in range(20):
        ...
        lo, hi = retime_bounds(events, beat=beat, left=left, right=right)
        if hi - lo < 2e-3:
            continue
        target = rng.uniform(lo + 1e-4, hi - 1e-4)
        out = retime(events, beat=beat, seconds=target, left=left, right=right)
